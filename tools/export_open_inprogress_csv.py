...
def export(max_rows: int = 50000) -> tuple[Path, Path, int]:
    ...
    csv_path = DATA_DIR / f"requests_open_inprogress_{len(rows)}.csv"
    json_path = DATA_DIR / f"requests_open_inprogress_{len(rows)}.json"
...
def main():
    ...
    print(f"Wrote {n} rows (Open/In Progress) to {csv_path} and {json_path}")
